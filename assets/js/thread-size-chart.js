// Behavior for /thread-size-chart/. Depends on letterDrillSizes and
...
(function () {
  ...
  // Clearance drills are the Close Fit (H12) column of ASME B18.2.8-1999 Table 2,
  // which explicitly covers #0 through 1.5 in (Sec. 1, Scope). Sizes not tabulated
  // directly (#12, 9/16) are computed the same way Table 2 itself was derived: nominal
  // major diameter + the Close Fit allowance from Table 1's size bracket, rounded to the
  // nearest standard drill. 1-3/4 in and 2 in fall outside that stated scope; their
  // clearance values extend the standard's own last allowance bracket (1-3/8, 1-1/2 in:
  // Close = +1/16 in) rather than a tabulated figure, since B18.2.8 defines nothing past
  // 1.5 in.
  //
  // Tap drills above 1 in aren't independently tabulated in any ASME/ISO standard we
  // could access, but they're not a guess either: every one sits at a constant offset
  // above the basic minor diameter D1 published in ASME B1.1 Tables 6/7, and that offset
  // is identical for every size sharing the same TPI -- e.g. all four 12-TPI UNF sizes
  // from 1 to 1-1/2 in (including the already-verified 1 in row) sit exactly 0.0277 in
  // above their B1.1 D1, no exceptions; the 7-TPI and 6-TPI UNC pairs hold their own
  // constant offsets (0.0140 in, 0.0085 in) just as exactly. That invariant is strong
  // evidence these come from a real historical tap-drill convention built on B1.1's own
  // geometry, not a rounded-to-a-nice-fraction placeholder.
  const americanThreads = [
    ['#0', 0.0600, 80, 'UNF', 0.046875, 0.0670],
    ['#1', 0.0730, 64, 'UNC', 0.0595, 0.0810],
    ['#2', 0.0860, 56, 'UNC', 0.0700, 0.09375],
    ['#3', 0.0990, 48, 'UNC', 0.0785, 0.1065],
    ['#4', 0.1120, 40, 'UNC', 0.0890, 0.1200],
    ['#5', 0.1250, 40, 'UNC', 0.1015, 0.140625],
    ['#6', 0.1380, 32, 'UNC', 0.1065, 0.1540],
    ['#8', 0.1640, 32, 'UNC', 0.1360, 0.1800],
    ['#10', 0.1900, 24, 'UNC', 0.1470, 0.2055],
    ['#12', 0.2160, 24, 'UNC', 0.1770, 0.2340],
    ['1/4', 0.2500, 20, 'UNC', 0.2010, 0.265625],
    ['1/4', 0.2500, 28, 'UNF', 0.2130, 0.265625],
    ['5/16', 0.3125, 18, 'UNC', 0.2570, 0.328125],
    ['5/16', 0.3125, 24, 'UNF', 0.2720, 0.328125],
    ['3/8', 0.3750, 16, 'UNC', 0.3125, 0.390625],
    ['3/8', 0.3750, 24, 'UNF', 0.3320, 0.390625],
    ['7/16', 0.4375, 14, 'UNC', 0.3680, 0.453125],
    ['7/16', 0.4375, 20, 'UNF', 0.390625, 0.453125],
    ['1/2', 0.5000, 13, 'UNC', 0.4219, 0.53125],
    ['1/2', 0.5000, 20, 'UNF', 0.4531, 0.53125],
    ['9/16', 0.5625, 12, 'UNC', 0.4844, 0.59375],
    ['9/16', 0.5625, 18, 'UNF', 0.515625, 0.59375],
    ['5/8', 0.6250, 11, 'UNC', 0.53125, 0.65625],
    ['5/8', 0.6250, 18, 'UNF', 0.578125, 0.65625],
    ['3/4', 0.7500, 10, 'UNC', 0.6562, 0.78125],
    ['3/4', 0.7500, 16, 'UNF', 0.6875, 0.78125],
    ['7/8', 0.8750, 9, 'UNC', 0.7656, 0.90625],
    ['7/8', 0.8750, 14, 'UNF', 0.8125, 0.90625],
    ['1', 1.0000, 8, 'UNC', 0.8750, 1.03125],
    ['1', 1.0000, 12, 'UNF', 0.9375, 1.03125],
    ['1 1/8', 1.1250, 7, 'UNC', 0.9844, 1.15625],
    ['1 1/8', 1.1250, 12, 'UNF', 1.0625, 1.15625],
    ['1 1/4', 1.2500, 7, 'UNC', 1.1094, 1.28125],
    ['1 1/4', 1.2500, 12, 'UNF', 1.1875, 1.28125],
    ['1 3/8', 1.3750, 6, 'UNC', 1.2031, 1.4375],
    ['1 3/8', 1.3750, 12, 'UNF', 1.3125, 1.4375],
    ['1 1/2', 1.5000, 6, 'UNC', 1.3281, 1.5625],
    ['1 1/2', 1.5000, 12, 'UNF', 1.4375, 1.5625],
    ['1 3/4', 1.7500, 5, 'UNC', 1.5469, 1.8125],
    ['2', 2.0000, 4.5, 'UNC', 1.7656, 2.0625],
  ];

  // Metric thread data
  // Format: [Thread Size (M), Major Diameter (mm), Pitch (mm), Tap Drill (mm), Clearance Drill (mm), Thread Type ('coarse' or 'fine')]
  // Clearance drills are the "fine" series (H12, ~ASME B18.2.8 Close Fit) of ISO
  // 273:1979 Table (dimensions in mm), read directly off the standard for every row
  // except M2.2, which ISO 273 and ASME B18.2.8 both omit as a non-preferred size.
  // M2.2's value (major + 0.2 mm) is not a guess: it's the exact allowance ISO 273
  // uses for both immediate neighbors, M2 (2.2, i.e. +0.2) and M2.5 (2.7, i.e. +0.2).
  const metricThreads = [
    // Coarse threads
    ...
    ['M12', 12.0, 1.75, 10.2, 13.0, 'coarse'],
    ['M14', 14.0, 2.0, 12.0, 15.0, 'coarse'],
    ['M16', 16.0, 2.0, 14.0, 17.0, 'coarse'],
    ['M18', 18.0, 2.5, 15.5, 19.0, 'coarse'],
    ['M20', 20.0, 2.5, 17.5, 21.0, 'coarse'],
    ['M22', 22.0, 2.5, 19.5, 23.0, 'coarse'],
    ['M24', 24.0, 3.0, 21.0, 25.0, 'coarse'],
    ['M27', 27.0, 3.0, 24.0, 28.0, 'coarse'],
    ['M30', 30.0, 3.5, 26.5, 31.0, 'coarse'],
    ['M33', 33.0, 3.5, 29.5, 34.0, 'coarse'],
    ['M36', 36.0, 4.0, 32.0, 37.0, 'coarse'],
    ['M39', 39.0, 4.0, 35.0, 40.0, 'coarse'],
    ['M42', 42.0, 4.5, 37.5, 43.0, 'coarse'],
    ['M45', 45.0, 4.5, 40.5, 46.0, 'coarse'],
    ['M48', 48.0, 5.0, 43.0, 50.0, 'coarse'],
    ['M52', 52.0, 5.0, 47.0, 54.0, 'coarse'],
    ['M56', 56.0, 5.5, 50.5, 58.0, 'coarse'],
    ['M60', 60.0, 5.5, 54.5, 62.0, 'coarse'],
    ['M64', 64.0, 6.0, 58.0, 66.0, 'coarse'],
    // Fine threads
    ['M3', 3.0, 0.35, 2.65, 3.2, 'fine'],
    ...
    ['M12', 12.0, 1.25, 10.75, 13.0, 'fine'],
    ['M14', 14.0, 1.5, 12.5, 15.0, 'fine'],
    ['M16', 16.0, 1.5, 14.5, 17.0, 'fine'],
    ['M18', 18.0, 1.5, 16.5, 19.0, 'fine'],
    ['M20', 20.0, 1.5, 18.5, 21.0, 'fine'],
    ['M22', 22.0, 1.5, 20.5, 23.0, 'fine'],
    ['M24', 24.0, 2.0, 22.0, 25.0, 'fine'],
    ['M27', 27.0, 2.0, 25.0, 28.0, 'fine'],
    ['M30', 30.0, 2.0, 28.0, 31.0, 'fine'],
    ['M33', 33.0, 2.0, 31.0, 34.0, 'fine'],
    ['M36', 36.0, 3.0, 33.0, 37.0, 'fine'],
    ['M39', 39.0, 3.0, 36.0, 40.0, 'fine'],
    ['M42', 42.0, 3.0, 39.0, 43.0, 'fine'],
    ['M45', 45.0, 3.0, 42.0, 46.0, 'fine'],
    ['M48', 48.0, 3.0, 45.0, 50.0, 'fine'],
    ['M52', 52.0, 3.0, 49.0, 54.0, 'fine'],
    ['M56', 56.0, 4.0, 52.0, 58.0, 'fine'],
    ['M60', 60.0, 4.0, 56.0, 62.0, 'fine'],
    ['M64', 64.0, 4.0, 60.0, 66.0, 'fine'],
  ];
    ...
    data.forEach((item) => {
      ...
      allColumns.forEach((col) => {
        ...
        switch (col) {
          ...
          case 'major-dia':
            if (item.type === 'American') {
              const majorDiaIn = item.majorDiaIn;
              const drills = findExactDrills(majorDiaIn);
              ...
              if (drills) {
                hasExactDrillMatch = drills.some(d => {
                  ...
                  return Math.abs(drillSize - majorDiaIn) < 0.000001;
                });
              }
              if (!hasExactDrillMatch) {
                // Use exact matching for major diameter (drillTolerance = 0)
                const originalTolerance = drillTolerance;
                drillTolerance = 0;
                fraction = decimalToFraction(majorDiaIn);
                drillTolerance = originalTolerance;
              }
              ...
              if (drills) {
                ...
                drills.forEach(drill => {
                  topLine.push(`${drill} (${majorDiaIn.toFixed(4)} in)`);
                });
              }
              if (fraction) {
                // Add fraction with decimal and "in" in parentheses
                topLine.push(`${fraction} (${majorDiaIn.toFixed(4)} in)`);
              }
              ...
              if (topLine.length > 0) {
                ...
                htmlParts.push(`${majorDiaIn.toFixed(4)} in`);
              }

              if (showMillimeters) {
                htmlParts.push(`${item.majorDiaMm.toFixed(2)} mm`);
              }
